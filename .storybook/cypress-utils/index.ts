...
export function createCommands(): void {
  ...
  cy.waitUntil(() => cy.window().then((win) => !!(win as any).mswWorker)).then(
    () => {
      debugger;
      (window as any).mswWorker.events.on("request:start", (request) => {
        registerRequest(request);
      });
      (window as any).mswWorker.on("request:end", (request) => {
  ...
  Cypress.on("test:before:run", () => {
    if (!(window as any).mswWorker) {
      return;
    }

    (window as any).resetHandlers();
    requests = {};
    routes = new Set();
  });
